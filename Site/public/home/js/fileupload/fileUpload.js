...
var uploadTools = {
     ...
     */
    "getShowFileType":function(isImg,fileType,fileName,isImgUrl,fileCodeId,uploadId){
		
        var showTypeStr="<div class='fileType'>"+fileType+"</div> <i class='iconfont icon-wenjian'></i><input type='hidden' name='"+uploadId+"[]' value='"+isImgUrl+"'/>";//默认显示类型
        if(isImg){
            if(isImgUrl!=null&&isImgUrl!="null"&&isImgUrl!=""){//图片显示类型
                showTypeStr = "<img src='"+isImgUrl+"'/><input type='hidden' name='"+uploadId+"[]' value='"+isImgUrl+"'/>";
            }
        }
        var modelStr="";
        modelStr+="<div class='fileItem mui-clearfix'  fileCodeId='"+fileCodeId+"'>";
        ...
        modelStr+="<i class='mui-icon mui-icon-closeempty'></i>";
        modelStr+="</div>";
    ...
    "fileIsExit":function(file,opt){
        var fileList = uploadFileList.getFileList(opt.uploadId);
        var ishave = false;
    ...
    "addFileList":function(fileList,opt){
        ...
        var fileListArray=uploadFileList.getFileList(opt.uploadId);
        ...
        for(var i=0;i<fileList.length;i++){
            ...
            if(fileTypeArray=="*"||uploadTools.isInArray(fileTypeStr,fileTypeArray)){
                ...
                if(imgtest.test(fileList[i].type)){
                    ...
                    var fileModel = uploadTools.getShowFileType(true,fileTypeUpcaseStr,fileList[i].name,imgUrlStr,fileListArray.length,uploadId);
                    $(boxJsObj).append(fileModel);
                }else{
                    var fileModel = uploadTools.getShowFileType(false,fileTypeUpcaseStr,fileList[i].name,imgUrlStr,fileListArray.length,uploadId);
                    $(boxJsObj).append(fileModel);
                }
        ...
        uploadFileList.setFileList(fileListArray,opt.uploadId);
...
var uploadEvent = {
    ...
    "cleanFileEvent":function(opt){
        ...
        uploadTools.cleanFilInputWithSelectFile(opt);
        uploadFileList.setFileList([],opt.uploadId);
        $("#"+uploadId+" .box").html("");
        uploadTools.initWithUpload(opt);//初始化上传
    }
}
...
var uploadFileList={
    ...
    "getFileList":function(uploadId){
		
		if(uploadId == undefined){
			return []
		}
		if(uploadFileList.fileList[uploadId]==undefined){
			uploadFileList.fileList[uploadId] = new Array();
		}
        return uploadFileList.fileList[uploadId];
		
    },
    "setFileList":function(fileList,uploadId){
		if(uploadFileList.fileList[uploadId]==undefined){
			uploadFileList.fileList[uploadId] = new Array();
		}
		uploadFileList.fileList[uploadId] = fileList;
    }
}
