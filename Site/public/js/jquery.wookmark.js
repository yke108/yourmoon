$.fn.wookmark=function(options){var widthSetter=$(this[0]);var getItemWidth=function(fixedWidth){if(fixedWidth===undefined){return widthSetter.outerWidth();}else{return fixedWidth;}}
if(!this.wookmarkOptions){this.wookmarkOptions=$.extend({container:$('body'),offset:2,autoResize:false,resizeDelay:50,resizeContainerHeight:true,flexibleItemWidth:false},options);}else if(options){this.wookmarkOptions=$.extend(this.wookmarkOptions,options);}
...
var columnWidth=getItemWidth(this.wookmarkOptions.itemWidth)+ this.wookmarkOptions.offset;var containerWidth=this.wookmarkOptions.container.width();var columns=Math.floor((containerWidth+this.wookmarkOptions.offset)/columnWidth);
if(this.wookmarkOptions.flexibleItemWidth){var overflow=containerWidth%columnWidth;if(overflow<columnWidth/2){columnWidth+=overflow/columns;var adjustedItemWidth=this.wookmarkOptions.itemWidth+ overflow/columns;this.width(adjustedItemWidth);}
else{columns+=1;columnWidth=containerWidth/columns;var adjustedItemWidth=columnWidth- this.wookmarkOptions.offset;this.width(adjustedItemWidth);}}
...
item.css({position:'absolute',top:shortest+'px',left:(shortestIndex*columnWidth+ offset)+'px'});heights[shortestIndex]=shortest+ item.outerHeight(true)+ this.wookmarkOptions.offset;bottom=Math.max(bottom,heights[shortestIndex]);this.wookmarkColumns[shortestIndex].push(item);}
...
if(this.wookmarkResizeMethod){$(window).unbind('resize',this.wookmarkResizeMethod);this.wookmarkOptions.container.unbind('refreshWookmark',this.wookmarkResizeMethod);}};this.wookmarkLayout();this.show();};