...
(function(b, d) {
	...
	function T(a, c, g) {
		if (c && c.indexOf(",") >= 0) h(c.split(","), function() {
			...
			var b = a[ma] || null;
			if (b) if (c === d) b in L && (h(L[b], function(c, g) {
	...
	function O(a, c) {
		return a = /\s+class\s*=/.test(a) ? a.replace(/(\s+class=["']?)([^"']*)(["']?[\s>])/, function(a, b, d, k) {
	...
	function H(a, c, g, b, d) {
		...
		var i = [],
			a = a.replace(/(\s*)<(\/)?([\w\-:]+)((?:\s+|(?:\s+[\w\-:]+)|(?:\s+[\w\-:]+=[^\s"'<>]+)|(?:\s+[\w\-:"]+="[^"]*")|(?:\s+[\w\-:"]+='[^']*'))*)(\/)?>(\s*)/g, function(a, f, n, l, o, j, s) {
				var f = f || "",
					n = n || "",
					m = l.toLowerCase(),
					r = o || "",
	...
	function Ra(a, c, g) {
		...
		function k(d, t) {
			for (var m = d.firstChild, s; m;) {
				...
				if (o >= 0) return !1;
				s = m.nextSibling;
				if (h > 0) if (m.nodeType == 1) if (l >= 0 && n <= 0) c && t.appendChild(m.cloneNode(!0)), g && e.push(m);
				else {
					var j;
	...
	if (!b.KindEditor) {
		...
		A(P, {
			...
			pos: function() {
				var a = this[0],
					c = 0,
					g = 0;
				if (a) if (a.getBoundingClientRect) a = a.getBoundingClientRect(), g = na(this.doc), c = a.left + g.x, g = a.top + g.y;
				else for (; a;) c += a.offsetLeft, g += a.offsetTop, a = a.offsetParent;
				return {
					x: fa(c),
					y: fa(g)
				}
			...
			index: function() {
				if (this.length < 1) return -1;
				for (var a = -1, c = this[0]; c;) a++, c = c.previousSibling;
				return a
			},
			prev: function() {
				if (this.length < 1) return null;
		...
		h("blur,focus,focusin,focusout,load,resize,scroll,unload,click,dblclick,mousedown,mouseup,mousemove,mouseover,mouseout,mouseenter,mouseleave,change,select,submit,keydown,keypress,keyup,error,contextmenu".split(","), function(a, c) {
		...
		A(aa, {
			...
			get: function(a) {
				...
				if (a && this.isControl()) return c = c.body.createControlRange(), c.addElement(this.startContainer.childNodes[this.startOffset]), c;
		...
		A(va, {
			...
			selection: function(a) {
				...
				this.sel = Ta(c);
				if (b) return this.range = Sa(b), i(this.range.startContainer).name == "html" && this.range.selectNodeContents(c.body).collapse(!1), this;
			...
			select: function(a) {
				...
				if (a && d.nodeType == 1 && b.collapsed) {
					...
					if (ka && (a = d.childNodes, i(d).isInline() || e > 0 && i(a[e - 1]).isInline() || a[e] && i(a[e]).isInline())) b.insertNode(k.createTextNode("\u200b")), l = !0
				}
				if (Y) try {
					h = b.get(!0), h.select()
				} catch (o) {} else l && b.collapse(!1), h = b.get(!0), c.removeAllRanges(), c.addRange(h), k !== document && (b = i(h.endContainer).pos(), f.scrollTo(b.x, b.y));
				f.focus();
				return this
			},
			wrap: function(a) {
				...
				i(a).scan(function(a) {
					if (!k && a == e.start) k = !0;
						...
						if (!d && c.type == 3 && m(a.nodeValue).length > 0) {
							...
							f = b;
							f = f.clone(!0);
							if (c.type == 3) Wa(f).append(c.clone(!1)), c.replaceWith(f);
							else {
								for (var a = c, h;
								(h = c.first()) && h.children().length == 1;) c = h;
								h = c.first();
								for (c = c.doc.createDocumentFragment(); h;) c.appendChild(h[0]), h = h.next();
								h = a.clone(!0);
								d = Wa(h);
								for (var l = h, n = !1; f;) {
			...
			val: function(a) {
				...
				if (a === "forecolor") return (a = this.commonNode({
					"*": ".color"
				})) && (b = a.css("color")), b = r(b), b === "" && (b = "default"), b.toLowerCase();
			...
			submit: function() {
				...
				c.bind("load", function() {
					...
					var b = i.iframeDoc(c),
						...
						e = "",
						f, e = d ? d.innerHTML : b.body.innerHTML,
						e = v(e);
		...
		Ba.prototype = {
			...
			updateState: function() {
				var a = this;
				h("justifyleft,justifycenter,justifyright,justifyfull,insertorderedlist,insertunorderedlist,subscript,superscript,bold,italic,underline,strikethrough".split(","), function(c, b) {
...
KindEditor.plugin("baidumap", function(b) {
	...
	d.clickToolbar("baidumap", function() {
		...
		var m = ['<div style="padding:10px 20px;"><div class="ke-header"><div class="ke-left">', f.address + ' <input id="kindeditor_plugin_map_address" name="address" class="ke-input-text" value="" style="width:200px;" /> ', '<span class="ke-button-common ke-button-outer">', '<input type="button" name="searchBtn" class="ke-button-common ke-button" value="' + f.search + '" />', '</span></div><div class="ke-right">', '<input type="checkbox" id="keInsertDynamicMap" name="insertDynamicMap" value="1" /> <label for="keInsertDynamicMap">' + f.insertDynamicMap + "</label>", '</div><div class="ke-clearfix"></div></div>', '<div class="ke-map" style="width:' + j + "px;height:" + e + 'px;"></div>', "</div>"].join(""),
			...
			v, p, r = b('<iframe class="ke-textarea" frameborder="0" src="' + d.pluginsPath + 'baidumap/map.html" style="width:' + j + "px;height:" + e + 'px;"></iframe>');
...
KindEditor.plugin("filemanager", function(b) {
	...
	var f = this,
		j = b.undef(f.fileManagerJson, f.basePath + "php/file_manager_json.php"),
		...
		function s(d) {
			...
			for (var n = d.file_list, m = 0, j = n.length; m < j; m++) {
				...
				var q = b('<img src="' + (e + (r.is_dir ? "folder-16.gif" : "file-16.gif")) + '" width="16" height="16" alt="' + r.filename + '" align="absmiddle" />'),
					q = b(p[0].insertCell(0)).addClass("ke-cell ke-name").append(q).append(document.createTextNode(" " + r.filename));
...
KindEditor.plugin("flash", function(b) {
	var d = this,
		...
		o = b.undef(d.uploadJson, d.basePath + "php/upload_json.php");
	d.plugin.flash = {
		edit: function() {
...
KindEditor.plugin("multiimage", function(b) {
	...
	var f = b.undef(d.uploadJson, d.basePath + "php/upload_json.php"),
...
(function() {
	...
	SWFUpload.prototype.unescapeFilePostParams = function(b) {
		...
		if (b != void 0) {
			for (var e in b.post) if (b.post.hasOwnProperty(e)) {
				j = e;
				for (var h;
				(h = d.exec(j)) !== null;) j = j.replace(h[0], String.fromCharCode(parseInt("0x" + h[1], 16)));
				f[j] = b.post[e]
			}
			b.post = f
		}
	...
	SWFUpload.prototype.returnUploadStart = function(b) {
		var d;
		if (typeof this.settings.upload_start_handler === "function") b = this.unescapeFilePostParams(b), d = this.settings.upload_start_handler.call(this, b);
	...
	SWFUpload.Console.writeLine = function(b) {
		...
		try {
			...
			if (!d) f = document.createElement("form"), document.getElementsByTagName("body")[0].appendChild(f), d = document.createElement("textarea"), d.id = "SWFUpload_Console", d.style.fontFamily = "monospace", d.setAttribute("wrap", "off"), d.wrap = "off", d.style.overflow = "auto", d.style.width = "700px", d.style.height = "350px", d.style.margin = "5px", f.appendChild(d);
			d.value += b + "\n";
...
KindEditor.plugin("quickformat", function(b) {
	...
	f.clickToolbar("quickformat", function() {
		...
		for (var e = f.edit.doc, h = f.cmd.range, m = b(e.body).first(), n, o = [], l = [], s = h.createBookmark(!0); m;) {
			n = m.next();
			var v = d(m);
			if (!v || v.name != "img") if (j[m.name] ? (m.html(m.html().replace(/^(\s|&nbsp;|\u3000)+/ig, "")), m.css("text-indent", "2em")) : l.push(m), !n || j[n.name] || j[m.name] && !j[n.name]) l.length > 0 && o.push(l), l = [];
			m = n
		}
	...
	e.plugin.table = {
		prop: function(n) {
					...
					yesBtn: {
						...
						click: function() {
							...
							if (d == 0 || !/^\d+$/.test(d)) alert(e.lang("invalidRows")), j[0].focus();
								...
								e.addBookmark()
							} else alert(e.lang("invalidBorder")), G[0].focus();
							else alert(e.lang("invalidSpacing")), A[0].focus();
							else alert(e.lang("invalidPadding")), q[0].focus();
							else alert(e.lang("invalidHeight")), r[0].focus();
							else alert(e.lang("invalidWidth")), p[0].focus()
						}
					}
				}).div,
			...
			if (!n && (u = e.plugin.getSelectedTable())) {
				...
				if (o !== void 0 && (I = /^(\d+)((?:px|%)*)$/.exec(o))) r.val(I[1]), D.val(I[2]);
		...
		colinsert: function(d) {
			...
			for (var h = 0, v = f.rows.length; h < v; h++) {
				var p = f.rows[h],
					d = p.insertCell(d);
				d.innerHTML = b.IE ? "" : "<br />";
				d = j(f, p, d)
