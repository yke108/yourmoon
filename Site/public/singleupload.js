	...
	$.hySingleImage = {
		...
		show:function(obj, data, no_close_link){
			...
			if(no_close_link != 1){
				...
				.css({'position':'absolute', 'bottom':'1px', 'right':'1px', 'font-size':'12px', 
					'padding':'3px','width':'12px','height':'12px','line-height':'12px',
					'margin-right':'auto',
						'border':'0', 'color':'red', 'background':'none', 'font-size':'14px',
						'font-weight':'bold'})
				.appendTo(wrap).bind('click',function(){
					var pobj = $(this).parent();
					pobj.children('input[type="hidden"]').val('');
					pobj.next('input[name="ad_picture"]').val('');
					$(this).remove();
					pobj.children('img').remove();
					pobj.children('div').show();
				});
			}//end if
		}
	}
